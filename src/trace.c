...
volatile U16 trace_state = 0;
volatile U16 trace_control = 0;
volatile U16 STORE_POS = 0;
U16 trace_counter = 0;
S16 trace_trigger = 0;
...
const S16 volatile *trace_source_addr[]  =
{
  /* 0 */    (S16*)NULL,
  /*******************************************************/
  /* MEASURE.C */
  ...
  /* 15 */  (S16*)&ident.Ident_peak_current,
...
void do_trace(void)
{
  static U16 trace_period = 0;
  S16 comp_param_local;
  
  par_trace.trace_display[0] = *(S16*)trace_source_addr[u16TraceParamNumber_1];
  par_trace.trace_display[1] = *(S16*)trace_source_addr[u16TraceParamNumber_2];
  ...
  if(_trace_running())    //if trace is running
  {
    ...
    else
    {
      if(!trace_period)
      {
        TRSB[STORE_POS][0] = par_trace.trace_display[0];
        TRSB[STORE_POS][1] = par_trace.trace_display[1];
        STORE_POS++;
