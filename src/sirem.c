...
static HRESULT CmdExe_ReadTraceData(U8* pFrame)
{
  ...
  switch(rd_trace_state)
  {
    ...
    case READ_TRACE_COMPOSE_FRAME:
      ...
      u32Index  = (U32)pFrame[3+0] << 16;
      u32Index |= (U32)pFrame[3+1] << 8;
      u32Index |= (U32)pFrame[3+2] << 0;
      
      /* length (number of words to read) */
      u32Array_length  = (U32)pFrame[6+0] << 16;
      u32Array_length |= (U32)pFrame[6+1] << 8;
      u32Array_length |= (U32)pFrame[6+2] << 0;
      
      if(u32Array_length == 0)    /* length = 0: return trace info */
      {
        pFrame[cSiremIndex_FrameStart]  = cSiremValueFrameStart;
    ...
    case READ_TRACE_TRMT_TRACE_ARRAY:
      if(u32Array_length > 0)
      {
        u32Chks += TRSB[u32Index];
        
        while( IF_UART_PutByte(TRSB[u32Index] >> 0) == 0 ) {};
        while( IF_UART_PutByte(TRSB[u32Index] >> 8) == 0 ) {};
