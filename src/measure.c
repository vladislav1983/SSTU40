...
BOOL DC_Autotunning(void)
{
  ...
  if((mp)->dc_average_counter)
  {
    ...
    (mp)->dc_sum += AdcReadChannel(ADC_CH1_CURRENT);
...
void Current_measure(void)
{
  ...
  ADC_Currnet_Raw = AdcReadChannel(ADC_CH1_CURRENT);
  Current_tmp = ADC_Currnet_Raw - (S16)(m1)->Curr_offset;
  Current_tmp *= 2; // et back to Q15 format -> 32767 = 25A

  Current_tmp = fmul_q15(Current_tmp,  CURRENT_MAX_mA);

  SuspendAllInterrupts();
  (m1)->Current = Current_tmp;
  ResumeAllInterrupts();
  
  if(_power_up_ok())
  {
    if(absi(m1->Current) > (mp)->Overcurr_limit)
    {
...
void measure_T2(void)
{
  ...
  (m2)->Actaul_Power = _builtin_divud(_builtin_muluu((id)->IDENT_MAX_RMS_Power, (cop)->power_half_periods), (1000u / 10u)); // 1000ms / 10ms 
}
/******************************************************************************/
/*
