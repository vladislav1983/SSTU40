...
#define serCONTINUEINIDLEMODE			0
...
void IF_Uart_Init(U32 u32Baudrate)
{
	...
	_sci_timer_start(sci_TIMER_TICKS); //<------ INIT Here sci timer
}
...
HRESULT IF_Uart_MsgGet(U8 *pDestinBuff)
{
    ...
    if( gRxMsg.bReady )		//if message ready
    {
        ...
        gRxMsg.bReady    = cFalse;    // there no anymore valid frame.
...
void IF_Uart_RxPurge(void)
{
     if( gRxMsg.bReady )	//if availible message
    {
        ...
        gRxMsg.bReady    = cFalse;    	// no more message
    }
...
void __attribute__((__interrupt__,no_auto_psv)) _U1RXInterrupt()
{
    ...
    {
        // read whole buffer to clear framing and parity flags (UART contain 4 level FIFO)
        while((U1STAbits.URXDA) && (u8DataCnt < 4))
        {
